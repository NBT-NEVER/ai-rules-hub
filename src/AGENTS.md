...
- 公式字符表示/表达、对话公式渲染版与 LaTeX 版、Word/PPT 公式变量说明格式、同目录 README 公式字符规则：`skills/formula-character-expression`
- Git 暂存、提交、分支管理、远程 `origin` 配置、GitHub 推送/拉取、代理检查、推送超时、大文件入库、Git LFS、提交归因与项目级版本管理：`skills/git-github-remote-management`
- 实验报告、算法说明、论文解读中的公式内容解释、方法说明、实现逻辑概述、课程式技术说明、结果分析、README 式中文项目介绍：`skills/lab-report-writer`；公式字符格式参考 `skills/formula-character-expression`
- 在对话中解释公式、需要先给渲染公式再给 LaTeX 版本、并逐项解释变量：`skills/formula-character-expression`；如还需要课程式文字说明，再组合 `skills/lab-report-writer`
- 与代码位于同一文件夹的 `README.md` 中需要写代码原理公式、LaTeX 变量说明或 GitHub 可渲染公式：`skills/code-project-rules` + `skills/formula-character-expression`
...
- 同一任务同时涉及实验报告正文公式字符和同目录 README 公式时：`skills/formula-character-expression` + `skills/lab-report-writer` + `skills/code-project-rules`
- 只在对话框中展示和讲解公式，不写入文档时：`skills/formula-character-expression`；如还需要课程式技术说明，再加 `skills/lab-report-writer`
- 远程推送 GitHub、`git add`、`git commit`、`git push`、修改远端地址、处理代理/超时/大文件/提交归因等项目管理任务：`skills/git-github-remote-management`
- 实验报告、技术说明、正文公式字符或对话公式说明需要减少 AI 味：`skills/formula-character-expression` + `skills/lab-report-writer` + `skills/academic-anti-ai-writing` + `skills/chinese-encoding-guard`
